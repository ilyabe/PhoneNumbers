        ...
        var event = {
            fill: function(el){
              ...
              $stars
                ...
                .filter(':eq(' + index + ')').addClass('hover').end();
              // Update the description text and label.
              if (summaryHover && !feedbackTimerId) {
                var summary = $("select option", $obj)[index + $cancel.size() + 1].text;
...
Drupal.behaviors.utility_bar_login_forms = function (context) {
  // TODO: Move these CSS changes into a conditional iOS CSS file
  if(navigator.platform == 'iPad' || navigator.platform == 'iPhone' || navigator.platform == 'iPod') {
    $('#page').css('position','absolute').css('top','30px');
    $("#utility-bar").css("bottom", null).css("top", 0); //put the utility bar at the top 
    $("#utility-bar #sign-in .pull-out").css('bottom', null).css('top','30px').css('left','-107px'); //put the pull-out below the utility bar and kick it a bit to the left
    $("#block-utility_bar-utility_bar_login_form h2.block-title").css('display','none');
    $("#utility-bar-wrapper").css('border-top-left-radius', '0px').css('border-top-right-radius', '0px').css('border-bottom-right-radius','5px').css('border-bottom-left-radius','5px'); //change the corner rounding to the bottom instead of the top 
  };


  // Toggle showing the forms block based on current visibility.
  $("#sign-in-link", context).not('.utility-processed').click(function () {
    // utility_bar_signin_form_hide|show refers to the entire block of code
    // which contains both the login and register forms.
    if ($('#sign-in-forms').is(':visible')) {
      utility_bar_signin_form_hide();
    }
    else {
      // Determine which form was last displayed before showing 
      // the signin block.
      var show_register = $('#sign-in').data('show_register');
      utility_bar_signin_form_show(show_register);
    }
    return false;
  }).addClass('utility-processed');


  // Add click event handlers to the main close link
  $('#sign-in-forms .close a', context).not('.utility-processed').click(function() {
    //for some reason slideToggle isn't playing nice with iOS in our environment, so use a less fancy transition
    if(navigator.platform == 'iPad' || navigator.platform == 'iPhone' || navigator.platform == 'iPod') {
      $('#sign-in-forms').css('display', 'none');
    }
  ...
  // Toggle the login and register forms
  $("#button-create-an-account", context).not('.utility-processed').click(function () {
    var curpath = location.pathname.replace(/^\/|\/$/g, '');
    if (curpath != 'user/register') {
      utility_bar_register_form_show();
    }
    else {
      // slideUp actually slides the block down. slideUp is  
      // just a close action.
      $('#sign-in-forms').slideUp('fast');
      $('#edit-profile-first-name').focus();
    }
    return false;
  }).addClass('utility-processed');
  ...
  // Toggle the login and register forms
  $("#sign-in-forms #button-sign-in", context).not('.utility-processed').click(function () {
    utility_bar_login_form_show();
    return false;
  }).addClass('utility-processed');
    
  // Update form actions to include URL anchor fragments  
  utility_bar_form_action_update();

  // Find the appropriate thank-you and display it on the page at the appropriate spot
  // var matches = location.href.search(/(m|p)=thank\-you/);
  if (location.href.indexOf('thank-you=m') != -1) {
    if ($('#control .error').length || $('#userreg-thanks').length) {
      // do nothing for now
    }
    else {
      var thanksmsg = '<div id="userreg-thanks">' + Drupal.settings.wh_userreg.wh_userreg_thanks_message + '</div>';
      $('#control').prepend(thanksmsg);
      $('#control .status').hide();
      $('.congreet_status').hide();
    }
  }
  
  // Add a thank you notifier to the user registration form action
  var thanks_type = (location.pathname.indexOf('petitions') > -1) ? 'p' : 'm';
  $('#utility-bar-form-register form').each(function(index) {
    var action = $(this).attr('action');
    if (action.indexOf('thank-you=') == -1) {
      action += '%2523thank-you=' + thanks_type;
      $(this).attr('action', action);
    }
  });
  
  // Check if captcha is available and enable
  if ($('div#recaptcha-ajax', context).length && $('div#recaptcha-ajax', context).not('.recaptcha-processed')) {
  	// Enable on all paths except /user/register, it has a captcha already
  	if (location.pathname.indexOf('/user/register') == -1) {
			Recaptcha.create(Drupal.settings.utility_bar.recaptcha.public_key, 'recaptcha-ajax', {
    		theme: Drupal.settings.utility_bar.recaptcha.theme,
      	callback: Recaptcha.focus_response_field
     	});
  	}
  	$('div#recaptcha-ajax', context).addClass('recaptcha-processed');
  } 
}
...
function utility_bar_form_action_update() {
  $('#sign-in-forms form').each(function(index) {
    var temppath = $(this).attr('action').split('?destination=');
    var lochash = (location.hash.length) ? location.hash : '';
    if (lochash.length) lochash = (lochash.indexOf('#!') == 0) ? '/' + location.hash.substr(1) : '%23' + location.hash.substr(1);
    var destination = location.pathname + lochash; // %2523 ?
    destination = destination.replace(/^\/|\/$/g, '');
    destination = (destination.indexOf("user") == 0) ? 'user' : destination;
    $(this).attr('action', temppath[0] + '?destination=' + destination);
  });

  $('form#user-register').attr('action', '/user/register?destination=user%23thank-you=m');
}


/**
 * Hide the signin block of forms (animate down).
 */
function utility_bar_signin_form_hide() {
  // Record which of the 2 forms is currently visible so that it 
  // can be restored to the current state if the signing block is
  // toggled closed and open again.
  var show_register = $('.create-account-block').is(':visible');
  $('#sign-in').data('show_register', show_register);
  
  // slideUp actually slides the block *down*. slideUp is
  // just a close action.
  $('#sign-in-forms').slideUp('fast');
  $('#sign-in').removeClass('sign-in-expand');
}

/**
 * This will lazy-load the utility bar register/login forms
 * from the ESI fragment on first request and inserts it into 
 * the DOM.
 */
function utility_bar_signin_form_show(show_register) {
  // show_register true to display the register form
  // instead of the default login form.
  var show_register = show_register || false;

  // Figure out if the ESI fragment has already been loaded by 
  // if a div from that fragment is present in the DOM.
  if ($(".sign-in-block").length) {
    utility_bar_display_form(show_register);
  }
  else {
    // Get the fragment and shove it into the DOM. Since .get() is 
    // asynchronous we need to dislay the form in an anonymous callback
    // function rather than calling it after the .get().
    $.get("/esi/block/utility_bar/utility_bar_login_form/0", function(data) {
      $('#utility-bar-login-forms').html(data);
      
      // we are doing this instead of Drupal.attachBehaviors() 
      // b/c we might be in a hybrid D6/D7 JS environemnt.
      jQuery.each(Drupal.behaviors, function() {
        if($.isFunction(this)) {
          this($('#utility-bar'));
        }
      ...
      // finally display the form now that it's loaded and behaving
      utility_bar_display_form(show_register);
    });
  }
...
/**
 * Display the signing block of forms, then call a function to
 * display a form within the signin block according to which form
 * is desired.
 */
function utility_bar_display_form(show_register) {
  // slideDown slides the block up. slideDown is 
  // just an open action.
  $('#sign-in-forms').slideDown('fast');
  $('#sign-in').addClass('sign-in-expand');
  if (show_register) {
    utility_bar_register_form_show();
  }
  else {
...
/**
 * Show the user register form after hiding the login form.
 */
function utility_bar_register_form_show() {
  $('.sign-in-block').fadeOut(100, function() {
    $('.create-account-block').fadeIn(100);
  });
}

/**
 * Show the login form after hiding the user register form.
 */
function utility_bar_login_form_show() {
  $('.create-account-block').fadeOut(100, function() {
    $('.sign-in-block').fadeIn(100);
  });
}
;
          ...
          if (!get_updates_submitted) {
            $.ajax({  
              ...
              url: "/get-email-updates",  
                data: $('#webform-client-form-618').serialize(),  
		beforeSend: function() { 
					...
					if (!get_updates_submitted) {
					  ...
					  $("#wh-forms-submit-news").click(function() {
						$.ajax({
							...
							data: $('#webform-client-form-'+Drupal.settings.updates_newsletter_nid).serialize(),
							beforeSend: function() {
								$('#get-email-updates-modal .modal-mid #webform').remove();
...
// If the page has a status message, change the class
  $('.messages.status').removeClass('messages').removeClass('status').addClass('congreet_status');
  
// If the page has a date error, relocate it
  var msg = $('div.congreet_date div.form-item div.error-message').text();
  $('div.congreet_date div.form-item div.error-message').remove();
  $('div.congreet_date').parent().append('<div class="congreet_date_error">'+msg+'</div>');

// Add the required field wrappers to the hidden by default fields as required
  $('label[for="edit-district"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-adoption-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-child-fullname"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-childbirth-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-baby-fullname"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-birthday80-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-birthday80-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-veteran70-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-veteran70-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-civilunion-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-partner-fullname"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-deceased-fullname"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-deceased-relationship"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-classreunion-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-classreunion-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-classreunion-institution"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-courtofhonor-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-reunion-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-reunion-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-reunion-familyname"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-girlscoutaward-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-photo-1"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-religious25-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-religious25-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-religious25-name"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-religiousmilestone-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-religiousmilestone-occasion"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-retirement-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-retirement-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-retirement-occupation"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-school-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-school-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-school-name"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-graduation-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-wedding-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-wedding50-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-wedding50-years"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-wedding70-date"]').wrap('<div class="required-field-wrapper">');
  $('label[for="edit-wedding70-years"]').wrap('<div class="required-field-wrapper">');

  if ($('#edit-salutation').val() == 'Representative') {
    $('#district').removeClass('hide_cell');
  }

  if (('#edit-item-requested') != "") {
    showSubItems();
  }

/**
 * Toggle whether or not the district textbox is displayed
 */
  $('#edit-salutation').change(function() {
    if (($('#edit-salutation').val()) == "Representative") {
      ...
      $('#district').addClass('hide_cell');
    }
  });


/**
 * Bind the char limit function to the text areas
 */
  $('#edit-deceased-furtherinfo').keyup(function() {
    congreetLimitChars('edit-deceased-furtherinfo', 200, 'deceased_charlimit');
  });

  $('#edit-retirement-furtherinfo').keyup(function() {
    congreetLimitChars('edit-retirement-furtherinfo', 200, 'retirement_charlimit');
  });

  $('#edit-illness-furtherinfo').keyup(function() {
    congreetLimitChars('edit-illness-furtherinfo', 200, 'illness_charlimit');
  });

  $('#edit-suggestions').keyup(function() {
    congreetLimitChars('edit-suggestions', 500, 'suggestions_charlimit');
  });


/**
 * Display the appropriate sub-fields for the selected item requested
 */
  $('#edit-item-requested').change(function() {
    $('.congreet_date_error').remove();
    $('.reqitem').addClass('hide_cell');
    $('.popup').addClass('hide_cell');;
    $('.extra_add').addClass('hide_cell');
    showSubItems();
  });
 ...
 *
 */
  $('#edit-recipient-prefix').change(function() {
    if ( $('#edit-recipient-prefix').val() == "Other (please specify)") {
      $('#prefix_other').removeClass('hide_cell');
    }
    else {
      $('#prefix_other').addClass('hide_cell');
    }
  });
 ...
 *
 */
  $('#edit-recipient-suffix').change(function() {
    if ( $('#edit-recipient-suffix').val() == "Other (please specify)") {
      $('#suffix_other').removeClass('hide_cell');
    }
    else {
      $('#suffix_other').addClass('hide_cell');
    }
  ...
  function showSubItems() {
    switch ($('#edit-item-requested').val()) {
      case 'Baby: Adoption of Child (sent after adoption)': {
          $('#adoption').removeClass('hide_cell');
          $('#popup2').removeClass('hide_cell');
          $('#parent2').removeClass('hide_cell');
          $('#child_fullname').removeClass('hide_cell');
          $('#edit-issue2').val("5_GREET_BABY");
          break;
      }
      case 'Baby: Birth of Child (sent after birth)': {
          $('#childbirth').removeClass('hide_cell');
          $('#popup2').removeClass('hide_cell');
          $('#parent2').removeClass('hide_cell');
          $('#baby_fullname').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_BABY');
          break;
      }
      case 'Birthday: 80+ years': {
          $('#birthday80').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_BIRTHDAY');
          break;
      }
      case 'Birthday: Veteran 70+ years': {
          $('#veteran70').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_BIRTHDAY');
          break;
      }
      case 'Civil Union (sent after ceremony)': {
          $('#civilunion').removeClass('hide_cell');
          $('#popup2').removeClass('hide_cell');
          $('#partner_fullname').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_WEDDING');
          break;
      }
       case 'Class Reunion: 10 years and every 5 years thereafter': {
          $('#classreunion').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_OTHER');
          break;
       }
       case 'Condolence (sent to next of kin only)': {
          $('#deceased').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_CONDOLENCE');
          break;
      }
       case 'Eagle Scout': {
          $('#eaglescout').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_OTHER');
          break;
      }
       case 'Family Reunion: 10 years and every 5 years thereafter': {
          $('#reunion').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_OTHER');
          break;
      }
      case 'Girl Scout Gold Award': {
          $('#girlscout').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_OTHER');
          break;
      }
      case 'Photograph': {
          $('#photos').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('9_PHOTO');
          break;
      }
      case 'Religious Institution Anniversary: 25 years and every 5 years thereafter': {
          $('#religious25').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val("5_GREET_RELIGIOUS");
          break;
      }
      case 'Religious Milestone': {
          $('#religious_milestone').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_RELIGIOUS');
          break;
      }
      case 'Retirement: 30+ years': {
          $('#retirement').removeClass('hide_cell');
          $('#popup3').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_RETIREMENT');
          break;
      }
      case 'School Anniversary: 100 years and every 25 years thereafter': {
          $('#school').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_OTHER');
          break;
      }
      case 'School Graduation': {
          $('#graduation').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_GRADUATION');
          break;
      }
      case 'Serious Illness': {
          $('#illness').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_ILLNESS');
          break;
      }
      case 'Wedding Day (sent after wedding)': {
          $('#wedding').removeClass('hide_cell');
          $('#popup2').removeClass('hide_cell');
          $('#partner_fullname').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_WEDDING');
          break;
      }
      case 'Wedding Anniversary: 50 or 60 years': {
          $('#wedding50').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#partner_fullname').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_WEDDING');
          break;
      }
      case 'Wedding Anniversary: 70+ years': {
          $('#wedding70').removeClass('hide_cell');
          $('#popup1').removeClass('hide_cell');
          $('#partner_fullname').removeClass('hide_cell');
          $('#edit-issue2').val('5_GREET_WEDDING');
          break;
  ...
  function congreetLimitChars(textid, limit, infodiv) {
    var text = $('#'+textid).val();
...
var wh_addthis_output = '<div id="addthis-container"><a class="addthis_button_email"><div id="addthis-email"></div></a><a class="addthis_button_twitter"><div id="addthis-twitter"></div></a><a class="addthis_button_facebook"><div id="addthis-share"></div></a><a class="addthis_button_compact"><div id="addthis-plus"></div></a></div>';
   ...
   $("#wh_addthis_output").html(wh_addthis_output);
   addthis.button("#wh_addthis_output");
  ...
  $("#wh-signup-submit").click(function() {
	if (wh_signup_validate()) { 
		...
		hidden_form_container
			   ...
			   $("#input[name*="+tmpList+"]").attr("checked","checked");   

			   $.ajax({  
				type: "POST",  
				url: "/get-email-updates-newsletters",  
				data: $('#wh-signup-tmp form[id^=webform-client-form]').serialize(),  
				beforeSend: function () {
...
;
Drupal.behaviors.wh_userreg_origin_populate = function (context) {
  $('input[name="origin"]').val(location.pathname.replace(/^\/|\/$/g, '') + location.search + location.hash);
};
;
...
Drupal.behaviors.wh_theme_ie6menu = function(context) {
  if ($.browser.msie && $.browser.version=="6.0") {
		...
		if (window.attachEvent) {
		 sfHover("topnav");
		 if (document.getElementById("categories-dropdown")) {
		   sfHover("categories-dropdown");
		 }
		}		
  }
};
