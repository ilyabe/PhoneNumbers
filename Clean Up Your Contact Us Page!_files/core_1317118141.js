...
{return true;}
var m2=s.match("^(https?|mailto|ftp|wais|file|gopher|telnet)://");return(null==m2)?false:true;}},isString:function(o)
...
{unSubscribeFunc();}}},subscribeArray);unSubscribeFunc=subscribeArray.pop();$iTXT.tmpl.dependsOn(name,false);},highestZIndex:function()
{var z=0;var elements=document.getElementsByTagName("*");var el=elements.length;for(var i=0;i<el;i++)
{var elmt=elements[i];z=Math.max(z,elmt.style.zIndex);}
...
{if($iTXT.core.Util.isArray(platform))
...
{$iTXT.debug.info($iTXT.debug.Category.CONTEXT,"Force Chunking is <b>ON</b>");}
...
{$iTXT.metrics.Misc.stopXfer=(new Date()).getTime();$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,'<b style="color: blue">ECH:</b> Data transfer took '+($iTXT.metrics.Misc.stopXfer-$iTXT.metrics.Misc.startXfer)+'ms.');}},_corsData:function(opts)
...
{opts.chunkKey=$iTXT.js.chunkKey||0;var contextUrl=this._appendChunkKey(opts.contextUrl,opts.chunkKey);var chunkUrl=this._appendChunkKey(opts.chunkUrl,opts.chunkKey);var data=encodeURIComponent(opts.data)||"";var chunkSize=$iTXT.core.Ajax.maxURLLength-chunkUrl.length;var contextSize=Math.max(0,$iTXT.core.Ajax.maxURLLength-contextUrl.length);var chunksArr=[];var chunksCounter=0;while(data.length>contextSize){$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Getting chunk "+chunksCounter);var dataChunk;if(data.length<chunkSize)
{$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Chunk remaining data");dataChunk=data;data='';}
else if("%"==data.charAt(chunkSize-1))
{$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Last character is % so truncating chunk");dataChunk=data.substring(0,chunkSize-1);data=data.substring(chunkSize-1);}
else if("%"==data.charAt(chunkSize-2))
{$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Second to last character is % so truncating chunk");dataChunk=data.substring(0,chunkSize-2);data=data.substring(chunkSize-2);}
else
{$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Chunk ok as it is");dataChunk=data.substring(0,chunkSize);data=data.substring(chunkSize);}
chunksArr.push(this._createChunk(chunkUrl,chunksCounter,dataChunk));chunksCounter++;}
$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,"Sending "+chunksCounter+" Chunks To <b>"+chunkUrl+"</b>");$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,"Using Chunk Key: <b>"+opts.chunkKey+"</b>");chunksArr.push(this._createChunk(contextUrl,chunksCounter,data));this._chunkSequential(opts,chunksArr);},_appendChunkKey:function(url,chunkKey){if(url.indexOf("?")==-1)
{url+="?";}
else
{url+="&";}
...
{var ro={};ro.left=$iTXT.core.Util.parseInt($iTXT.core.Util.getStyle(this,'padding-left','paddingLeft'),0);ro.top=$iTXT.core.Util.parseInt($iTXT.core.Util.getStyle(this,'padding-top','paddingTop'),0);ro.right=$iTXT.core.Util.parseInt($iTXT.core.Util.getStyle(this,'padding-right','paddingRight'),0);ro.bottom=$iTXT.core.Util.parseInt($iTXT.core.Util.getStyle(this,'padding-bottom','paddingBottom'),0);return ro;}}}