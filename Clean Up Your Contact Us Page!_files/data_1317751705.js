...
{var undefined;var $itxtUtil=$iTXT.core.Util;$iTXT.data.Context={BAD_EXCL:1,BAD_SHORT:2,BAD_SML_IMG:4,BAD_IAB_IMG:8,BAD_NOT_CONTENT:16,allowedFields:{c:'itxtAllowed',h:'itxtAllowed',n:'itxtAllowed',p:'itxtAllowed',t:'itxtAllowed',u:'itxtAllowed',v:'itxtAllowed',w:'itxtAllowed',x:'itxtAllowed'},changedContentSections:{},contextFull:true,currentDepth:0,dynamicContentMode:false,eligibleElementsTotal:-1,harvestCount:0,harvestedCount:0,lastTime:0,lastTimeOut:-1,metrics:0,modifiedTrees:[],nodeAssessments:{},nodeTextLengths:{},pageElementsTotal:-1,pageTitle:'',paragraphNodes:[],processedElementsCount:0,textNodes:[],timedOut:false,timings:{},treeObjectMode:false,visScanned:false,Node:$iTXT.core.Class.create({c:null,h:null,n:null,p:null,t:null,u:null,v:null,w:null,x:null,tn:'',init:function(n,params,parent)
...
var didA=false;var thisA=$iTXT.data.Context.loadAssessment(n);if((!thisA||!$itxtUtil.isObject(thisA))&&params.assessor&&params.assessor.assess)
...
if(""==this.t)
{delete this.t;}
$iTXT.data.Context.currentDepth++;this._getChildren(n,params,assessment);this._setVisibility(n,params,assessment,parent);this._setHookable(n,params,assessment,parent);this._setParagraph(n,params,assessment,parent);this._setWeight(n,params,assessment,parent);this._setContent(n,params,assessment,parent);if(this._isBad(n,assessment.hasHooknodes))
...
{var badTags={'applet':true,'embed':true,'iframe':true,'img':true,'object':true,'script':true,'style':true,'xml':true};var childTags=[],childTexts=[],childAttrs=[];var nT=n.nodeType,nTn=n.tagName.toLowerCase();if(n.nodeType==$itxtUtil.ELEMENT_NODE)
...
{if(!parent)
{parent={};}
var cont=null;var nName=n.nodeName;var nValue=$itxtUtil.cleanString(n.nodeValue);if(('title'==parent.t||'title'==this.t)&&n.innerText)
...
{if(this._relatedBPNs(cN,params))
{inst.p=1;this.p=null;this._addParagraphNode();}}
var t=this.t||parent.t;if(params.mintextlength>0&&content.length<params.mintextlength&&1!=inst.p&&1!=this.p&&1!=parent.p&&"h"!=t&&"title"!=t&&this._childTags.length==0&&"meta"!=t)
...
{var std=((this.t&&'std'==this.t)||(parent.t&&'std'==parent.t)||(!parent.t&&!this.t))?true:false;if(std)
...
{inst.t='l';}}
std=true;var tN=n.tagName||pN.tagName;if(null!=tN)
{if(tN.toLowerCase)
{tN=tN.toLowerCase();}
if('strong'==tN||'b'==tN)
{this.t='b';}
...
{keepCalmAndCarryOn=$iTXT.data.Dom.visibilityStates[v];}
...
{if(assessment.isBreaknode||assessment.isUnbreaknode||this._lenOK(n,params))
{this.p=1;}}}}}
if(1==this.p)
...
{this.t=parent.t;}}},_setVisibility:function(n,params,assessment,parent)
{if(!assessment.isVisibilitynode||this.h||!n.nodeType||$itxtUtil.ELEMENT_NODE!=n.nodeType||assessment.isSkipnode)
{return;}
...
{$iTXT.data.Context.getPseudoURL(this.v);}}
if(!$iTXT.data.Context.visScanned)
{$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<span style="color:#ff6600;"><b>Web 2.0:</b> </span> Setting handler on '+$iTXT.data.Dom.getElementSignature(n,true)+' ('+this._nId+').');$iTXT.data.Dom.setOnPropChange(n,function(e)
...
{if($iTXT.data.Context.visScanned)
{return;}
this.metrics=this.params.get('metrics',0);this.setContextFull();this.setDynamicContentMode();this.setTreeObjectMode();this.setPageTitle();var startNodes=this.getStartNodes();var timeoutName='timeout.light';if(this.contextFull)
...
this.preprocessExcludeNodes(this.params);var oParams={};var sA=new $iTXT.data.Dom.SelectorAssessor();var pL=this.params.list(),params=[];for(var i=0,len=pL.length;i<len;i++)
...
{oParams.visscanwholepage=!!('*'==this.params.get('visibilitynodes',[])[0]);$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<b style="color:#ff6600;">Web 2.0:</b> Visibility checking enabled.');}
if(!this.timings||!$itxtUtil.isObject(this.timings))
{this.timings={};}
this.timings.cStart=new Date().getTime();var trees=[];this.params.set('oparams',oParams);for(var i=0,len=startNodes.length;i<len;i++)
...
$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,dbNodes);}}},getProcessedPercentage:function()
...
if("v1"==cM)
{if(null!=hookNodes&&hookNodes.length>0)
{var assessor=new $iTXT.data.Dom.SelectorAssessor();assessor.add($iTXT.data.Dom.parseVMNode(hookNodes),'startnode');}
...
if(($iTXT.core.Util.isString(nd.id)&&'intellitxt'==nd.id.toLowerCase())||(nd.getAttribute('name')&&'intellitxt'==nd.getAttribute('name').toLowerCase())||assessment.isStartnode)
{if(this.metrics&$iTXT.debug.Util.ECH_TEXTNODES_DUMP)
...
if(rA.length==0)
{$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<b style="color:red;">WARNING:</b> We are in "v1" mode and no start nodes were found.');}}
else
...
{var nd=bN[o];if(this.metrics&$iTXT.debug.Util.ECH_TEXTNODES_DUMP)
...
{$iTXT.data.Context.visScanned=true;var nId=$iTXT.data.Dom.setNodeId(n);var cN=$iTXT.data.Dom.nodeIds[nId];var vSec=cN.v;if(!$itxtUtil.isObject(cN))
{return;}
var oParams=this.params.get('oparams');$iTXT.data.Context.visScanned=true;this.timings.cStart=new Date().getTime();this.harvestCount++;this.modifiedTrees[this.modifiedTrees.length]={nodeId:nId,fieldName:'v',fieldValue:cN.v,newTree:new this.Node(n,oParams).simplify()};this.timings.cEnd=new Date().getTime();var doneFlag=this.changedContentSections[vSec].sent||false;var vState=$iTXT.data.Dom.isVisible(n);$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<span style="color:#ff6600;"><b>Web 2.0:</b> REACT </span> '+$iTXT.data.Dom.getElementSignature(n,true)+' is now '+((vState)?'':'in')+'visible (section '+vSec+').');if(vState)
{$iTXT.glob.params.set('refurl',this.getPseudoURL(vSec),100);$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<span style="color:#ff6600;"><b>Web 2.0:</b> </span> Using sectional URL "'+$iTXT.glob.params.get('refurl')+'" for section '+vSec+'.');var spliced=this.modifyTree();this.lastTime=(this.timings.cEnd-this.timings.cStart);if($iTXT.debug.Util.isLoggingOn())
{var sTree=$itxtUtil.serialiseJSON(this.tree,this.allowedFields);var json=(this.metrics&$iTXT.debug.Util.ECH_XPAND_JSON)?$itxtUtil.formatJSONString(sTree,true):sTree;$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<b style="color:#ff6600;">ECH:</b>  '+((this.contextFull)?'FULL':'LIGHT')+' <i>sectional</i> content harvesting completed after '+this.lastTime+'ms.  '+this.textNodes.length+' nodes in '+(this.paragraphNodes.length)+' paragraphs are now hookable.');$iTXT.debug.info($iTXT.debug.Category.CONTEXT,'<b style="color:#ff6600;">ECH:</b> This produces '+(sTree.length)+' bytes of JSON:<br /><span style="color:green;">'+json+'</span><br />');}
if(this.metrics&$iTXT.debug.Util.ECH_TEXTNODES_DUMP)
{var hnl=$iTXT.data.Context.textNodes.length;if(hnl>0)
...
{if(thisH[sz])
{var kN='is'+int.substr(0,1).toUpperCase()+int.substr(1);rO[kN]=true;match=true;}}
else
...
{for(var o in tO)
{var tOo=tO[o];if($itxtUtil.isArray(tOo))
{cands=cands.concat(tOo);}}}}
if(cands.length>0&&$iTXT.data.Dom.findSelector(s,cands,1))
{var kN='is'+int.substr(0,1).toUpperCase()+int.substr(1);rO[kN]=true;match=true;}}
if(true==match)
{if(this._othHash[int]&&this._othHash[int][sz]&&$itxtUtil.isArray(this._othHash[int][sz]))
...
{return;}
var nId=this.setNodeId(elmt);if(!this.propChangeIntervals[nId])
...
$iTXT.data.LogEventType={ADVIEW:1,TOOLTIPCLICK:2,HOOKCLICK:3,MONITOR:4,HOOKLOG:5}
...
$iTXT.data.AdLogger=$iTXT.core.Class.create({server:"",qavTID:-1,trkDrps:null,pWind:null,pWindName:"",pWindUrl:"",demoMode:false,duplicateAdViews:[],allowHookClick:true,logsInProgress:0,init:function(o)
{if($iTXT.data.al)
{return;}
...
url+=extraParams;}};$iTXT.fire("$iTXT:adlogger:before:log",cb);$iTXT.debug.debug($iTXT.debug.Category.ADLOGGER,((ret)?"Building Event: ":"Logging Event: ")+url);var o=o||{};$iTXT.data.al.logsInProgress++;if(!o.onLoad)
{o.onLoad=function()
...
dids+=addetails.did;adids+=addetails.adid;syids+=addetails.syid;apids+=addetails.pid;upw+=addetails.uf;upwr+=addetails.ur;var tOff=hk.getPosition();kwp+=tOff.left+","+tOff.top+";";}}}
...
{return;}}}}}
$iTXT.debug.info($iTXT.debug.Category.ADLOGGER,"Logging <b>Page View</b> ("+$itxtUtil.serialiseJSON(qps)+")");this._logQS(this._buildQS(qps));},_ttOpn:function(e)
{var o=e.data||{};var ad=o.advert||$iTXT.ui.tt.currentAdvert;if(ad)
...
return;var o=e.data||{};var ad=o.advert||$iTXT.ui.tt.currentAdvert;if(ad)
...
if(!this.duplicateAdViews[ad.did]){this.duplicateAdViews[ad.did]=1;}}},_ttMsOvr:function(e)
...
{return;}}
var ll=ad.params.get("LIVELOOKUP",0);var llif=ad.params.get("LIVELOOKUPFINISHED",0);var logsInProgress=$iTXT.data.al.logsInProgress>0;if((ll&&!llif)||logsInProgress)
...
{$iTXT.debug.info($iTXT.debug.Category.ADLOGGER,'<b style="color:red;">Hook Click NOT Allowed as allowHookClick is false</b>');return;}
